...
//matt added description of 3 and 4-bit bimodal predictors
 *
 *	BPred3bit:  a simple direct mapped bimodal predictor
 *
 *		This predictor has a table of three bit saturating counters.
 *		Where counter states 0, 1, 2, & 3 are predict not taken and
 *		counter states 4, 5, 6, & 7 are predict taken, the per-branch counters
 *		are incremented on taken branches and decremented on
 *		no taken branches.  One BTB entry per counter.
 *
 *	BPred4bit:  a simple direct mapped bimodal predictor
 *
 *		This predictor has a table of four bit saturating counters.
 *		Where counter states 0, 1, 2, 3, 4, 5, 6, & 7 are predict not taken and
 *		counter states 8, 9, 10, 11, 12, 13, 14, & 15 are predict taken, the per-branch counters
 *		are incremented on taken branches and decremented on
 *		no taken branches.  One BTB entry per counter.
//matt
 *
...
//matt added BPred3bit and BPred4bit enums
  BPred3bit,			/* 3-bit saturating cntr pred (dir mapped) */
  BPred4bit,			/* 4-bit saturating cntr pred (dir mapped) */ 
//matt 
