...
sim_check_options(struct opt_odb_t *odb, int argc, char **argv)
{
  if (!mystricmp(pred_type, "taken"))
    ...
    {
//matt update argument check. REQUIRE that # be specified!
      if (bimod_nelt != 2)
...
//matt choose bimodal predictor enum based on second config param, or default to BPred2bit
      enum bpred_class class;
      switch(bimod_config[1]) {
        case 4:
          class = BPred4bit;
          break;
        case 3:
