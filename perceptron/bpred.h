...
enum bpred_class {
  BPredPerceptron,              /* Jiminez Perceptron implementation */
  BPredPerceptronMod,           /* Modification to the Jiminez Perceptron implementation */
  BPredMLP,			/* MLP predictor */
...
struct bpred_dir_t {
  ...
  union {
    ...
    /* Perceptron Predictor: This is Jiminez Implementation*/
    struct {
      // table of perceptrons.  hardcoded size for now...
      signed int weight_table[2048][500];
      int weight_table_size;
      int num_weight_bits;
      
      // length of global history 
      int shift_width;
      int bhr[100];// again hardcoded for now...
      int local_bhr[2048][100];// local bhr for each branch

      // this is the saved prediction from last computation
...
struct bpred_t {
  ...
  struct {
    ...
    //struct bpred_dir_t *mlp; /*mlp predictor */
  } dirpred;
