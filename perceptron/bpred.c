	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  /*
  Jiminez Perceptron Implementataion
  l1size = size of perceptron table
  l2size = bit for weight entry.  basically creates an upper/lower bound for weights
  shift_width = how long our global branch history register is
  */
  ...
  /*
  Our modified perceptron implementataion
  l1size = size of perceptron table
  l2size = bit for weight entry.  basically creates an upper/lower bound for weights
  shift_width = how long our global branch history register is
  */
  case BPredPerceptronMod:
    pred->dirpred.bimod = 
      bpred_dir_create(class, l1size, l2size, shift_width, 0);
    break;
  ...
  case BPred2Level:
  case BPred2bit:
  /* Perceptron will behave the same as the others */
  case BPredPerceptron:
  case BPredPerceptronMod:
    {
      int i;
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  /* I should allocate mem here eventually so its not hard coded...*/
  case BPredPerceptron: 
  {
    if (!l1size)
      fatal("Number of perceptrons, `%d', must be defined", l1size);
    pred_dir->config.perceptron.weight_table_size = l1size;

    if (!l2size)
      fatal("Weigh Bits, `%d', must be defined", l2size);
    pred_dir->config.perceptron.num_weight_bits = l2size;
    ...
    // each perceptron will contain shift_width weights
    // (i.e. one weight for each history value)
    for(int i = 0; i < pred_dir->config.perceptron.weight_table_size; i++) {
      for(int j = 0; j < shift_width + 1; j++) {
        pred_dir->config.perceptron.weight_table[i][j] = 0;
      }
    }
    // Initialize global BHR to taken.  I suppose we could randomly
    // initialize this also.  That would be closer to typical NN.
    // bhr[0] is bias 
    for(int j = 0; j < shift_width + 1; j++) {
      pred_dir->config.perceptron.bhr[j] = 1;
    }
    break;
  }
  case BPredPerceptronMod:
  {
    ...
    // Initialize perceptron table
    for(int i = 0; i < pred_dir->config.perceptron.weight_table_size; i++) {
      // shift_width weights for global history, shift_width weights for local history, 1 bias weight
      for(int j = 0; j < (shift_width * 2) + 1; j++) {
        pred_dir->config.perceptron.weight_table[i][j] = 0;
      }
    }
    // Initialize global BHR to taken. 
    for(int j = 0; j < shift_width + 1; j++) {
      pred_dir->config.perceptron.bhr[j] = 1;
    }
    // initialize local bhr to 1
    for(int i = 0; i < pred_dir->config.perceptron.weight_table_size;i++) {
      for(int j = 0 ; j < shift_width; j++) {
        pred_dir->config.perceptron.local_bhr[i][j] = 1;
      }
    }
    break;
  }
  ...
  FILE *stream)			/* output stream */
{
  ...
  case BPredPerceptron:
    fprintf(stream, "pred_dir: %s: perceptron table: %d entries, weight bits: %d\n",
      name, pred_dir->config.perceptron.weight_table_size, pred_dir->config.perceptron.num_weight_bits);
    break;
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  case BPredPerceptronMod:
    bpred_dir_config (pred->dirpred.bimod, "modified perceptron", stream);
    fprintf(stream, "btb: %d sets x %d associativity", 
	    pred->btb.sets, pred->btb.assoc);
    fprintf(stream, "ret_stack: %d entries", pred->retstack.size);
    break;
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  switch (pred->class)
    ...
    case BPredPerceptron:
      ...
      break;
    case BPredPerceptronMod:
      name = "bpred_perceptron_modified";
      break;
    default:
      panic("bogus branch predictor class");
    }
...
/* predicts a branch direction i.e. taken / not taken */
		 ...
		 md_addr_t baddr)		/* branch address */
{
  ...
  switch (pred_dir->class) {
    ...
    case BPredPerceptron:
      {
      /* Compute the output of our perceptron */
      int prediction;
      prediction = 0; 
   
      // Get index into our table of perceptrons using the address
      // of the current branch
      int index;
      index  = (baddr >> MD_BR_SHIFT) % pred_dir->config.perceptron.weight_table_size;
      int numWeights;
      int shiftWidth = pred_dir->config.perceptron.shift_width;
      // loop through weights and compute output
      // sum of weight[i] * bhr[i]
      // recall that bhr[0] is the bias input
      for(int i = 0; i < shiftWidth + 1; i++) {
        prediction += (pred_dir->config.perceptron.weight_table[index][i] * pred_dir->config.perceptron.bhr[i]);
      }
      if(pred_dir->class == BPredPerceptronMod) {
        // also need to include local history
        for(int i = 0 ; i < shiftWidth; i++) { 
          prediction += (pred_dir->config.perceptron.weight_table[index][i + (shiftWidth + 1)] * pred_dir->config.perceptron.local_bhr[index][i]);
        }
      }
      
      // save output
      pred_dir->config.perceptron.prediction = prediction;
      // return pointer to prediction
      p = (unsigned char *)&pred_dir->config.perceptron.prediction;
      }
      break;
    case BPredTaken:
    case BPredNotTaken:
					 ...
					 * used on mispredict recovery */
{
  ...
  switch (pred->class) {
      ...
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
      ...
      break;
    case BPredTaken:
      return btarget; // simple just return target address
    case BPredNotTaken:
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if (dir_update_ptr->pdir1)
    {
      if(pred->class == BPredPerceptron || pred->class == BPredPerceptronMod) {
        // Peceptron Update Logic:
        // (baddr >> MD_BR_SHIFT) & (pred->dirpred.twolev->config.two.l1size - 1)
        // get index into our table of perceptrons
        index = (baddr >> MD_BR_SHIFT) % pred->dirpred.bimod->config.perceptron.weight_table_size;

        // this is the previous prediction made
        int prediction = pred->dirpred.bimod->config.perceptron.prediction;
        //printf("Prediction: %d, Real: %d\n", prediction, taken);

        // from Jiminez paper, a value theta which we will use to determine
        // if we should update
        int theta = 1.93 * pred->dirpred.bimod->config.perceptron.shift_width+ 14;
        int shiftWidth = pred->dirpred.bimod->config.perceptron.shift_width; 
        // if our prediction is smaller than some theta (i.e. it was a weak prediction
        // OR branch was taken and our prediction was negative (not taken)
        // OR branch was not taken and our prediction was positive (taken)
        if(abs(prediction) <= theta || ((taken != 0) && prediction < 0) || ((taken == 0) && prediction > 0)) {
          // update weights for this perceptron
          // recall bhr[0] is bias
          for(i = 0 ; i < shiftWidth + 1; i++) {
            if((pred->dirpred.bimod->config.perceptron.bhr[i] == 1 && taken != 0) || (pred->dirpred.bimod->config.perceptron.bhr[i] == -1 && taken == 0)) {
              // input (branch history) was representetative of current output.  in other words
              // history repeated itself.  so we will update the weight of this 
              // point in history
              pred->dirpred.bimod->config.perceptron.weight_table[index][i]++;
            } else {
              // history was incorrect in determining the current output. decrement
              // this weight
              pred->dirpred.bimod->config.perceptron.weight_table[index][i]--;
            }
            // restrict to proper number of bits
            if(pred->dirpred.bimod->config.perceptron.weight_table[index][i] > 127) { 
              pred->dirpred.bimod->config.perceptron.weight_table[index][i] = 127;
            } else if (pred->dirpred.bimod->config.perceptron.weight_table[index][i] < -128) {
              pred->dirpred.bimod->config.perceptron.weight_table[index][i] = -128;
            }
          }
          if(pred->class == BPredPerceptronMod) {
            // update local weights also
            int offset = shiftWidth + 1;
            for(i = 0; i < shiftWidth; i++) {
              if((pred->dirpred.bimod->config.perceptron.local_bhr[index][i] == 1 && taken != 0) || (pred->dirpred.bimod->config.perceptron.local_bhr[i] == -1 && taken == 0)) {
                pred->dirpred.bimod->config.perceptron.weight_table[index][i + offset]++;
              } else {
                pred->dirpred.bimod->config.perceptron.weight_table[index][i + offset]--;
              }
              if(pred->dirpred.bimod->config.perceptron.weight_table[index][i + offset] > 127) {
                pred->dirpred.bimod->config.perceptron.weight_table[index][i + offset] = 127;
              } else if (pred->dirpred.bimod->config.perceptron.weight_table[index][i + offset] < -128) {
                pred->dirpred.bimod->config.perceptron.weight_table[index][i + offset] = -128;
             }
           }
          }   
        }
        ...
        // Update BHR
        // recall that bhr[0] is always 1 for bias
        // shift eerything to the left 
        for(i = 1; i < shiftWidth - 2 + 1; i++) {
          pred->dirpred.bimod->config.perceptron.bhr[i] = pred->dirpred.bimod->config.perceptron.bhr[i+1];
        }
        // set newest value
        if(taken != 0) {
          pred->dirpred.bimod->config.perceptron.bhr[shiftWidth - 1 + 1] = 1;
         } else {
          pred->dirpred.bimod->config.perceptron.bhr[shiftWidth - 1 + 1] = -1;
        }
        if(pred->class == BPredPerceptronMod) {
          for(i = 0; i < shiftWidth - 2 ; i++) {
             pred->dirpred.bimod->config.perceptron.local_bhr[index][i] = pred->dirpred.bimod->config.perceptron.local_bhr[index][i+1];
          }
          if(taken != 0) { 
            pred->dirpred.bimod->config.perceptron.local_bhr[index][shiftWidth - 1] = 1;
          } else {
            pred->dirpred.bimod->config.perceptron.local_bhr[index][shiftWidth - 1] = -1;
          }
        }
            
        // and were done with perceptron         
         
      
      } else {
        // This is original code for Bimodal update
        if (taken)
  	  {
