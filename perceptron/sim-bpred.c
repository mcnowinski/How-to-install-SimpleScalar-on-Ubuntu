...
static int perceptron_config[3] = 
  { /* perceptron table size */2048, /* bits per weight */8, /* hist length */8};
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  /* Jiminez Perceptron implementation */
  opt_reg_int_list(odb, "-bpred:perceptron",
                   "perceptron predictor config "
		   "(<num_perceptrons> <num_weight_bits> <hist_size>)",
                   perceptron_config, perceptron_nelt, &perceptron_nelt,
		   /* default */perceptron_config,
                   /* print */TRUE, /* format */NULL, /* !accrue */FALSE);

  /* Modified perceptron implementation */
  opt_reg_int_list(odb, "-bpred:modpercept",
                   "modified perceptron predictor config "
		   "(<num_perceptrons> <num_weight_bits> <hist_size>)",
                   perceptron_config, perceptron_nelt, &perceptron_nelt,
		   /* default */perceptron_config,
                   /* print */TRUE, /* format */NULL, /* !accrue */FALSE);
...
sim_check_options(struct opt_odb_t *odb, int argc, char **argv)
{
  if (!mystricmp(pred_type, "taken"))
  ...
  else if (!mystricmp(pred_type, "perceptron"))
    {
      if(perceptron_nelt != 3)
        fatal("bad perceptron pred config (<num_perceptrons> <weight_bits> <hist_size>)");
      if (btb_nelt != 2)
        fatal("bad btb config (<num_sets> <associativity>)");
      ...
      pred = bpred_create(BPredPerceptron,
                          /* bimod table size */0,
                          /* num perceptrons */ perceptron_config[0],
                          /* num bits in weight */ perceptron_config[1],
                          /* meta table size */0,
  ...
  else if (!mystricmp(pred_type, "modpercept"))
    {
      if(perceptron_nelt != 3)
        fatal("bad perceptron pred config (<num_perceptrons> <weight_bits> <hist_size>)");
      if (btb_nelt != 2)
      ...
      /* Jiminez Perceptron implementation */
      pred = bpred_create(BPredPerceptronMod,
                          /* bimod table size */0,
                          /* num perceptrons */ perceptron_config[0],
                          /* num bits in weight */ perceptron_config[1],
                          /* meta table size */0,
